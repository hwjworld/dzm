...
function bindEvent() {
    ...
    temp.control.ove.addEventListener("viewchanging",
    function() {
        var i = {};
        if (this.isOpen()) {
            i.value = 1
...
function hidInputOverline() {
    ...
    var c = [];
    for (var b = 0; b < a.length; b++) { (a[b].type == "checkbox" || a[b].type == "radio" || a[b].type == "button") ? c.push(a[b]) : null
    }
...
function input_focus(c, a, b) {
    ...
    c.style.color = "#000"; (c.value != a) ? null: c.value = b
}
function input_blur(c, a, b) {
    c.style.color = "#8c8c8c"; (c.value == b || c.value.length <= 0) ? c.value = a: null
}
...
Fe.extend(MapSign.prototype, {
    ...
    createIcon: function() {
        ...
        var b = new BMap.Icon("images/us_mk_icon.png", new BMap.Size(c.w, c.h), {
